...
- shaku
- Arc は所有。Service が依存オブジェクトの所有権を共有している
- Arc::clone() のたびにアトミックな参照カウントのインクリメントが走る
- Service はアプリ起動時に一度だけ作られ、全リクエストで共有される

- nject
- &'a は借用。依存オブジェクトを「ちょっと借りてるだけ」
- 参照カウントなし。ポインタ1本のみなのでコストゼロ
- Service はリクエストのたびに作り直す（ただしポインタ2本の構造体なので極めて軽量）

借用元が消えると困るなと言う感じ。

- [lab-rust-di/nject-0-4-5](https://github.com/hideyuki-hori/lab-rust-di/tree/main/nject-0-4-5)

### その他

JavaJavaしいshakuは洗練された形って気がしてて、他のライブラリもshakuに似てるんじゃないか(未確認)という気持ちになり飽きてしまった。
もしかするとやるかもしれない。
